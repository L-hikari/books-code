...
loadShaderFile(baseUrl + 'matrix.vert', baseUrl + 'matrix.frag')
.then(main);
...
function main([VSHADER_SOURCE, FSHADER_SOURCE]) {
  ...
  const u_need_trans = gl.getUniformLocation(gl.program, 'u_need_trans');
  if (!u_need_trans) {
    console.log('Failed to get the storage location of u_need_trans');
    return -1;
  }
  ...
  .then(([image1, image2, image3, image4]) => {
    ...
    gl.uniform1i(u_need_trans, 1);
    gl.uniformMatrix4fv(u_transform, false, matrix);
    gl.drawArrays(gl.TRIANGLES, 12, 6);
    gl.uniform1i(u_need_trans, 0);
