...
loadShaderFile(baseUrl + 'scale.vert', baseUrl + 'scale.frag')
.then(main);
...
function main([VSHADER_SOURCE, FSHADER_SOURCE]) {
  ...
  const u_need_scale = gl.getUniformLocation(gl.program, 'u_need_scale');
  if (!u_need_scale) {
    console.log('Failed to get the storage location of u_need_scale');
    return -1;
  }
  ...
  .then(([image1, image2, image3, image4]) => {
    ...
    gl.uniform1i(u_need_scale, 1);
    gl.drawArrays(gl.TRIANGLES, 12, 6);
    gl.uniform1i(u_need_scale, 0);
